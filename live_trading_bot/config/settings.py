...
def _apply_db_params(settings: Settings) -> None:
    ...
    # Only apply DB trading pairs when TRADING_PAIRS was NOT set via env var.
    # The env var represents an explicit operator/harness override and must
    # take precedence over whatever the daily tuner wrote to param_snapshots.
    # Without this guard, two subprocesses started at slightly different times
    # (dry before live, or after a daily_tune.py run) can load different symbol
    # sets, causing the strategy's market-regime pre-pass to diverge and
    # producing opposite signals for shared symbols.
    if isinstance(trading_pairs, list) and not os.getenv("TRADING_PAIRS"):
